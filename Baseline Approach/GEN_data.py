...
M = 30 #number of knowledge points
DIFFICULTY = [random.randint(3,10) for i in range(M)]
DIFF_VAR = 2
DENSITY = 0.4
...
with open("generated_data.csv", mode="w") as generated_data:
    ...
    dwriter.writerow(["ID", "ethnicity", "gender", "age", "Trace of learning"])
    for i in range(N):
        ...
        for s in range(steps):
            ...
            time = random.randint(DIFFICULTY[cur_node]-DIFF_VAR, DIFFICULTY[cur_node]+DIFF_VAR)
            learning_sequence.append((cur_node, time))
            ...
            for (_,y) in DEPENDENCY.out_edges(cur_node):
                ...
                if avail: 
                    avail_kps.append(y)
        dwriter.writerow([str(i), ethnicity, gender, str(age), str(learning_sequence)])
